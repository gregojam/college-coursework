...
(define value-of-cps
  (lambda (exp env-cps k)
    (union-case exp expr
      ...
      [(mult x1 x2)
       (value-of-cps x1 env-cps (kt_mult-outer-k x2 env-cps k))]
      [(sub1 x)
       (value-of-cps x env-cps (kt_sub1-k k))]
      [(zero x)
       (value-of-cps x env-cps (kt_zero-k k))]
      [(if test conseq alt)
       (value-of-cps test env-cps (kt_if-k conseq alt env-cps k))]
      [(letcc body)
       (value-of-cps body (envr_extend k env-cps) k)]
      [(throw kexp vexp)
       (value-of-cps kexp env-cps (kt_throw-k vexp env-cps))]
      [(let exp body)
       (value-of-cps exp env-cps (kt_let-k body env-cps k))]
      [(lambda body)
       (apply-k k (clos_closure body env-cps))]
      [(app rator rand)
       (value-of-cps rator env-cps (kt_app-outer-k rand env-cps k))])))
  ...
  (λ (k v)
    (union-case k kt
      ...
      [(mult-inner-k v^ k^)
       (apply-k k^ (* v^ v))]
      [(mult-outer-k x2^ env-cps^ k^)
       (value-of-cps x2^ env-cps^ (kt_mult-inner-k v k^))]
      [(sub1-k k^)
       (apply-k k^ (sub1 v))]
      [(zero-k k^)
       (apply-k k^ (zero? v))]
      ...
      [(app-outer-k rand^ env-cps^ k^)
       (value-of-cps rand^ env-cps^ (kt_app-inner-k v k^))])))
