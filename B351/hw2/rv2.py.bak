...
class MyRobot:
    ...
    def findPathFrom(self, starts, exit):
        shortPath = []
        for start in starts:
    ...
    #        print
            ...
            if len(shortPath) == 0:
                shortPath = path
            elif len(shortPath) > len(path):
                shortPath = path
        return shortPath
    
    def getPath(self):
        starts = self.findStarts()
        exits = self.findExits()
        if len(exits):
            paths = []
            for exit in exits:
                paths.append(self.findPathFrom(starts, exit))
            optimal = paths[0]
            for path in paths:
                if len(path) == 0:
                    continue
